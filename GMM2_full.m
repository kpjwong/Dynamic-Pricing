...
load sim_data.mat;
N = 100000; iter = 10;
U = rand(N,2); param_mat = zeros(9,iter);
theta0 = [10;-4;-15;-30;-3;-1;-5;-1;0.8];
options = optimset('Display', 'iter', 'PlotFcns', @optimplotx, 'MaxIter', 1200, 'OutputFcn', @outfun);

for k = 1:iter
    k
    gen_data;
    param_mat(:,k) = fminsearch(@(param)mom_dist(param, est_data, U, z2zz, wait_mta_zz, mta_travel_time), theta0, options);
    param_mat(:,k)
end
...
function d = mom_dist(param, data, U, z2zz, wait_mta_zz, mta_travel_time)
    ...
    for n = 1:N
        mta_w(n) = wait_mta_zz(20*(data(n,2)-1) + zz(n));
        mta_t(n) = mta_travel_time(z1(n),data(n,4));
    end
    
    U_taxi = beta(1)*taxi_P + gamma(1)*data(:,6) + delta(1)*(taxi_T-data(:,6));
    U_uber = beta(1)*uber_P + gamma(1)*data(:,6) + delta(1)*(uber_T-data(:,6));
